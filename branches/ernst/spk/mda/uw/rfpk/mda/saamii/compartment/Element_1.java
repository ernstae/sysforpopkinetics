 ...
 * @author  Jiaji Du
 */
public abstract class Element_1 implements Cloneable
{ 
    ...
    protected static class Compartment extends Element
    {
        public Compartment(int x, int y, Model model)
        {
            ...
            super.yCenter = y;
            super.model = model;
            if(Model.deletedElements.size() > 0)
                number = ((Integer)Model.deletedElements.remove(0)).intValue();
            else
                number = ++Model.nElement;
            name = "COMP" + number;
        }
    ...
    {
        public Input(Element.Compartment compartment, Model model)
        {
            super.model = model;
            this.compartment = compartment;
            xCenter = compartment.xCenter - 33;
            yCenter = compartment.yCenter + 33;
            if(!Model.isClone)
            {
                if(model.deletedInputs.size() > 0)
                    number = ((Integer)model.deletedInputs.remove(0)).intValue();
                else
                    number = ++model.nInput;
                name = "I" + compartment.number;
            }            
        }
        ...
        public void draw(Graphics2D gc2D) 
        {
            ...
            gc2D.translate(-xCenter + 14, -yCenter + 14);
            gc2D.translate(compartment.xCenter, compartment.yCenter);
            int x = xCenter - compartment.xCenter;
            int y = yCenter - compartment.yCenter;
            double d = Math.sqrt(x*x + y*y);
            gc2D.setStroke(new BasicStroke(1.0f));
            gc2D.drawLine((int)(x*26/d), (int)(y*26/d), (int)(x - x*8/d), (int)(y - y*8/d));
        ...
        protected Element.Compartment compartment;
    }
    
    protected static class Sample extends Element
    {
        public Sample(Element.Compartment compartment, Model model)
        {
            super.model = model;
            this.compartment = compartment;
            if(!Model.isClone)
            {
                if(model.deletedSamples.size() > 0)
                    number = ((Integer)model.deletedSamples.remove(0)).intValue();
                else
                    number = ++model.nSample;
                name = "S" + number;
            }
           
            // Find number of samples alread applied to the compartment.
            int nSample = 0;
            for(int i = 0; i < model.samples.size(); i++)
                if(compartment == ((Element.Sample)model.samples.get(i)).compartment)
                    nSample++;
            xCenter = compartment.xCenter + 40 + 16 * nSample;
            yCenter = compartment.yCenter - 40;
        }
        ...
        public void draw(Graphics2D gc2D) 
        {
            ...
            {
                gc2D.setColor(Color.yellow);
                gc2D.fillOval(x - 6, y - 6, 12, 12);
            }
            gc2D.setColor(Color.red);
            gc2D.drawLine((int)(x*26/d), (int)(y*26/d), (int)(x - x*4/d), (int)(y - y*4/d));
            gc2D.drawOval(x - 4, y - 4, 8, 8);
            gc2D.translate(-compartment.xCenter, -compartment.yCenter);
            drawName(gc2D);
        }
