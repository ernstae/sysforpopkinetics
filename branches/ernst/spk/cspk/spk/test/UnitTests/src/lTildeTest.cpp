...
class FullDUserModelLTildeTest : public SpkModel<double>
{
    ...
    const int _nYi;
public:
    FullDUserModelLTildeTest(int nA, int nB, int nYi)
      : _nA(nA), _nB(nB), _nYi(nYi), _a(nA), _b(nB)
    {};    


    ~FullDUserModelLTildeTest(){};
private:
 ...
 *
 *
 * Test cases for lTilde
 *
 * Author: Sachiko Honda
 *
 *************************************************************************/

Test* lTildeTest::suite()
{
  TestSuite *suiteOfTests = new TestSuite("lTildeTest");

  suiteOfTests->addTest(new TestCaller<lTildeTest>("testLaplace",         &lTildeTest::testLaplace));
  suiteOfTests->addTest(new TestCaller<lTildeTest>("testHessian",         &lTildeTest::testHessian));
  suiteOfTests->addTest(new TestCaller<lTildeTest>("testNaiveFirstOrder", &lTildeTest::testNaiveFirstOrder));
  suiteOfTests->addTest(new TestCaller<lTildeTest>("cmpAllObjectives",    &lTildeTest::cmpAllObjectives));

    return suiteOfTests;
}

void lTildeTest::tearDown()
{
}
//
// This test verifies the three objective values are all the same.
// 
void lTildeTest::testLaplace()
{
  diagDTest(MODIFIED_LAPLACE, laplaceBOut, laplaceLOut, laplaceL_alpOut, laplaceLi_alpOut);
}
void lTildeTest::testHessian()
{
  diagDTest(EXPECTED_HESSIAN, foceBOut, foceLOut, foceL_alpOut, foceLi_alpOut);
}
void lTildeTest::testNaiveFirstOrder()
{
  diagDTest(NAIVE_FIRST_ORDER, foBOut,foLOut, foL_alpOut, foLi_alpOut);
}
void lTildeTest::cmpAllObjectives()
{ 
  int i,j;
...
void lTildeTest::diagDTest(enum Objective whichObjective,
                      DoubleMatrix &bOut,
                      double &LOut,
