...
\\
& \leq &
( 1 - \theta )^2 | R(x, a, b) |_2^2 
+ (1 - \theta) \theta \alpha | R(x, a, b) |_2 avg [ R ( x , a , b ) ] 
+ \theta^2 \alpha^2 avg [ R( x , a , b ) ]
O( \theta^2 )
\\
& \leq &
[ ( 1 - \theta)^2 + (1 - \theta) \theta \alpha  +  \theta^2 \alpha^2  ] f(0)
+
O( \theta^2 )
\end{array}
\] $$
...
$latex \[
	f ( \lambda ) - f(0) \leq  - \lambda \frac{1}{2} f (0) (1 - \alpha )
\] $$
...
{	bool ok    = true;
	...
	CppAD::vector<double> Rp(9);
	CppAD::vector<double> Rhs(3);
	CppAD::vector<double> Result(3);

	// compute average residual
	double avg = ( (*u - *x)*(*b) + (*x - *l)*(*a) ) / 2.;

	// first row of Rp
	Rp[0]  = - *b;
	Rp[1]  = 0.;
	Rp[2]  = *u - *x;
	Rhs[0] = - (*u - *x)*(*b) + alpha * avg; 

	// second row of Rp
	Rp[3]  = *a;
	Rp[4]  = *x - *l;
	Rp[5]  = 0.;
	Rhs[1] = - (*x - *l)*(*a) + alpha * avg; 
	...
	double logdet;
	CppAD::LuSolve(3, 1, Rp, Rhs, Result, logdet);
	...
	ok &= CppAD::NearEqual(lambda,   lamOut, 1e-12, 1e-12);
	ok &= CppAD::NearEqual(Result[0], *xOut, 1e-12, 1e-12);
	ok &= CppAD::NearEqual(Result[1], *aOut, 1e-12, 1e-12);
	ok &= CppAD::NearEqual(Result[2], *bOut, 1e-12, 1e-12);
...
# ifndef NDEBUG
	size_t j;
	// check that if lambda = 1, partial of Lagragian w.r.t. x is zero
	for(i = 0; i < n; i++)
	{	xOut[i] = x[i] + dx[i];
		aOut[i] = a[i] + da[i];
		bOut[i] = b[i] + db[i];
	...
	{	double sum = 0;
		for(j = 0; j < n; j++)
			sum += Q[i * n + j] * xOut[j];
		sum += r[i] - aOut[i] + bOut[i];
		QN01BoxUnknownError( fabs(sum) <= abserr, "Next" );
	}
# endif
	...
	{	for(i = 0; i < n; i++)
		{	xOut[i] = x[i] + lambda * dx[i];
			...
			QN01BoxUnknownError( l[i] < xOut[i], "Next" );
			QN01BoxUnknownError( u[i] > xOut[i], "Next" );
			QN01BoxUnknownError( 0.   < aOut[i], "Next" );
			QN01BoxUnknownError( 0.   < bOut[i], "Next" );
		}
		fs = Residual(n, Q, r, l, u, xOut, aOut, bOut, ftmp);

		bool small = fabs(fs - f0) <= 
			1e1 * DBL_EPSILON * (fabs(f0) + fabs(fs));
		QN01BoxUnknownError( (! nearZero) || small, "Next" );
		if( small )
		{	nearZero = true;
			for(i = 0; i < n; i++)
			{	xOut[i] = x[i];
