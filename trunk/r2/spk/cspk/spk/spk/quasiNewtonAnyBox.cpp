...
The value $math%xOut%$$ is accepted as an estimate for $math%xHat%$$ if 
$math%
  ...
  DoubleMatrix*          pdrowF_xOut )
{
  ...
  if ( !isWarmStartAnyBox )
  {
    ...
    if ( nMaxIterAnyBox > 0 )
    {
      ...
      initHessApprox( nObjParFree, epsilon, gScaled, hScaled );
    }
  }
  else
  {
 ...
 * 
 * epsilon
 * convergence criteria for x relative to bounds
 *
 * gCurr
  ...
  double*        h       )
{
  ...
  for ( i = 0; i < n; i++ )
  {
    h[i * n + i] =  fabs( gCurr[i] ) * std::min(100., 1e-2 / epsilon);
