         ...
         )
{
    ...
    {
        // Only include the D term in the expected Hessian if requested.
        if ( withD )
        {
        ...
        {
            symmetrize(add( H2, H3), *expectedHessianTilde);
	}
    }
    ...
    if( expectedHessianTilde_alp )
        *expectedHessianTilde_alp = expectedHessian_alp(model,dvecAlp,dvecB,dvecBStep,withD,dmatRInv,R_bApprox,f_bApprox, A);
}
         ...
         const DoubleMatrix A[])
{
    ...
    DoubleMatrix dmatDInv_a;
    if ( withD )
    {
      model.indParVarianceInv_popPar( DInv_a );
      dmatDInv_a.fromValarray( DInv_a, nA );
    ...
    // Only include the D term in the derivative if requested.
    if ( withD )
    {
        return add(add(dmatDInv_a,H_b234),H_b567);
    }
    else
    {
        return add(H_b234,H_b567);
    }
}
/*************************************************************************
 *
