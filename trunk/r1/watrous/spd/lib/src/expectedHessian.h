...
class ExpectedHessian : public std::binary_function<DoubleMatrix, DoubleMatrix, DoubleMatrix>
{
    ...
    public:
        ExpectedHessian(SpkModel *m, const DoubleMatrix& bStep, bool withD)
            : _model(m),  _bStep(bStep), _withD(withD)
        {}
        ~ExpectedHessian() throw() 
        {}
        ExpectedHessian(const ExpectedHessian& right)
            : _model(right._model), _bStep(right._bStep), _withD(right._withD)
        ...
        const DoubleMatrix operator()(const DoubleMatrix& dvecAlp, const DoubleMatrix& dvecB) const
        {
            ...
            _model->setIndPar(b);
            expectedHessian(*_model, dvecAlp, dvecB, _bStep, &expectedHessianTilde, 0, 0, _withD);
...
class ExpectedHessianValarray : public std::binary_function< SPK_VA::valarray<double>, SPK_VA::valarray<double>, SPK_VA::valarray<double> >
{
    ...
    public:
        ExpectedHessianValarray(SpkModel *m, const SPK_VA::valarray<double>& bStep, bool withD)
            : _model(m),  _bStep(bStep), _withD(withD)
        {}
        ...
        const SPK_VA::valarray<double> operator()(const SPK_VA::valarray<double>& a, const SPK_VA::valarray<double>& b) const
        {
            ...
            _model->setPopPar(a);
            _model->setIndPar(b);
            expectedHessian(*_model, DoubleMatrix( a ), DoubleMatrix( b ), DoubleMatrix( _bStep ), &expectedHessianTilde, 0, 0, _withD);
