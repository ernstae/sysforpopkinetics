# ifndef QN01Box_MemoryIncluded
# define QN01Box_MemoryIncluded
/*
-----------------------------------------------------------------------
...
an program $xref/Error/$$ will occur).
...
namespace QN01Box {
	...
	public:
		// constructor
		explicit inline Memory(size_t n) 
		: p(new Type[n + 2]) , ntot(n + 2), next(1)
		{	// electric fence to check for memory overwrite	
		...
		{	// check the total amount of allocated memory
			QN01BoxUsageError(next+1 == ntot, "Memory",
				"did not use all allocated memory" );
	
			// check for a memory overwrite
			QN01BoxUsageError( *first == 255, "Memory",
				"write to memory before what is allocated" );
			QN01BoxUsageError( *last  == 254, "Memory",
				"write of memory after what was allocated" );
		...
		{	// allocate the next chunck of memory
			next = next + m;
			QN01BoxUsageError(next < ntot, "Memory",
				"attempt to use more than allocated memory" );
			return p + next - m;
		}
