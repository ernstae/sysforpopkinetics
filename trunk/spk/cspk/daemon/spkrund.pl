...
sub reaper {
	...
	# Did the run exit normally?
	if (-f $filename_results) {
	    my $end_code = $child_exit_value == 0 ? "srun" : "serr";
	    my $results;
	    open(FH, $filename_results)
		or death('emerg', "can't open $filename_results");
	    read(FH, $results, -s FH);
	    close(FH);
	    &end_job($dbh, $job_id, $end_code, $report)
		or death('emerg', "job_id=$job_id: $Spkdb::errstr");
	    my $msg = $child_exit_value == 0 ?
		"normally" : "with software errors";
	    syslog('info', "job_id=$job_id terminated $msg");
	}
	# Did the run die because it was aborted by an operator
        # suspecting a bug?
