...
NonmemTranslator::NonmemTranslator( DOMDocument* sourceIn, DOMDocument* dataIn )
  : ClientTranslator ( sourceIn, dataIn ),
    ...
    fDataSet_h( "DataSet_h" ),
    fDataSet_cpp( "DataSet_cpp" ),
    header( "// THIS FILE IS GENERATED BY THE ASPK COMPILER" ),
...
void NonmemTranslator::generateIndData() const
{
  ...
  if( oIndData_h.good() )
      ...
      for( ; p != labels->end(); p++ )
	{
      ...
      for( ; p != labels->end(); p++ )
	{
	  oIndData_h << "const std::valarray<T> " << *p << ";" << endl;
	  if( ( synonym = table.findi( *p )->synonym ) != "" )
	    oIndData_h << "const std::valarray<T> " << synonym << ";" << endl;
	}

      const map<const string, Symbol> * t = table.getTable();
      map<const string, Symbol>::const_iterator m = t->begin();
      for( ; m != t->end(); m++ )
	{
	    if( find( labels->begin(), labels->end(), m->second.name ) == labels->end() )
	    {
	      oIndData_h << "std::valarray<T> " << m->second.name << ";" << endl;
	      oIndData_h << "std::valarray<T> " << m->second.name << "_tmp;" << endl;
	    }
	}
  ...
  ofstream oIndData_cpp( fIndData_cpp );
  if( oIndData_cpp.good() )
    {
      oIndData_cpp << header << " <" << __FILE__ << ">" << endl;

      oIndData_cpp << "#include \"IndData.h\"" << endl;
      oIndData_cpp << "#include <valarray>" << endl;
      oIndData_cpp << "using namespace std;" << endl;

      oIndData_cpp << "template <class T> IndData<T>::IndData( int nIn";
      const vector<string> * labels = table.getLabels();
      vector<string>::const_iterator p = labels->begin();
      for( ; p != labels->end(); p++ )
	{
	  oIndData_cpp << "," << endl;
	  oIndData_cpp << "const std::valarray<T> & " << *p << "In";
	}
      oIndData_cpp << ")" << endl;
      oIndData_cpp << ": n( nIn )";
      string synonym;
      ...
      for( ; m != t->end(); m++ )
	{
	    ...
	    {
	      oIndData_cpp << "," << endl;
	      oIndData_cpp << m->second.name << "( nIn )";
              oIndData_cpp << "," << endl;
	      oIndData_cpp << m->second.name << "_tmp( nIn )";
	    }
	}
      ...
      oIndData_cpp << endl;
      oIndData_cpp << "template <class T> IndData<T>::~IndData(){}" << endl;
      oIndData_cpp << "template <class T> IndData<T>::IndData(){}" << endl;
      oIndData_cpp << "template <class T> IndData<T>::IndData( const IndData& ){}" << endl;
      oIndData_cpp << "template <class T> IndData<T>& IndData<T>::operator=( const IndData& ){}" << endl;
    }
  else
    {
      assert( false );
...
void NonmemTranslator::generateDataSet( int pop_size ) const
{
  ofstream oDataSet_h( fDataSet_h );
  if( oDataSet_h.good() )
    {
      oDataSet_h << header << " <" << __FILE__ << ">" << endl;
      ...
      oDataSet_h << "IndData * set[" << pop_size << "];" << endl;
      oDataSet_h << endl;
  ...
  ofstream oDataSet_cpp( fDataSet_cpp );
  if( oDataSet_cpp.good() )
    {
      oDataSet_cpp << header << " <" << __FILE__ << ">" << endl;

      oDataSet_cpp << "#include <vector>" << endl;
      oDataSet_cpp << "#include \"DataSet.h\"" << endl;
      oDataSet_cpp << endl;
      oDataSet_cpp << "using namespace std;" << endl;
      oDataSet_cpp << endl;

      oDataSet_cpp << "DataSet::DataSet()" << endl;
      oDataSet_cpp << "{" << endl;
      ...
      for( int who=0; who < pop_size; who++ )
	{
          ...
          oDataSet_cpp << "//------------------------------------" << endl;
          oDataSet_cpp << "// Subject <" << id << "> " << endl;
	  oDataSet_cpp << "// # of sampling points = " << nRecords << endl;
          oDataSet_cpp << "//------------------------------------" << endl;
	  pLabel = labels->begin();
	  for( int i=0; pLabel != labels->end(), i<nLabels; i++, pLabel++ )
	    {
	      const Symbol * s = table.findi( *pLabel );
              ...
              oDataSet_cpp << "T " << carray_name << "[] = { ";
	      for( int j=0; j<nRecords; j++ )
		{
		  if( j > 0 )
		    oDataSet_cpp << ", ";
		  oDataSet_cpp << s->initial[who][j];
		}
	      oDataSet_cpp << " };" << endl;
	      oDataSet_cpp << "valarray<T> " << valarray_name << "(" << nRecords << ", " << carray_name << ");" << endl;
	    }
	  oDataSet_cpp << "set[" << who << "] = new IndData( " << nRecords << ", ";
	  pLabel = labels->begin();
	  for( int i=0; pLabel != labels->end(), i<nLabels; i++, pLabel++ )
	    {
	      if( i>0 )
      ...
      oDataSet_cpp << "DataSet::~DataSet(){}" << endl;
      oDataSet_cpp << "DataSet::DataSet( const DataSet& ){}" << endl;
      oDataSet_cpp << "DataSet& DataSet::operator=( const DataSet& ){}" << endl;
    }
  else
    {
      assert( false );
