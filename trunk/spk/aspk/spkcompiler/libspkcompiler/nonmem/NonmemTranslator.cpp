...
void NonmemTranslator::translate( DOMDocument* tree )
{
  ...
  DOMElement * driverTree = dynamic_cast<DOMElement*>( tree->getElementsByTagName( X("driver") )->item(0) );
  assert( driverTree != NULL );
  
  int nIndividuals = read_nonmem_driver( driverTree, ourSpk, ourNonmem );
  /*
  //
