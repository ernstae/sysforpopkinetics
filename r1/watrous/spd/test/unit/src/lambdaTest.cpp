 ...
 *
 *
 * This class performs the unit test for the function lambda.
 *
...
Test* lambdaTest::suite()
{
  ...
  suiteOfTests->addTest( new TestCaller<lambdaTest>(
    "scalarModelNoXCommonTest", &lambdaTest::scalarModelNoXCommonTest ) );

  suiteOfTests->addTest( new TestCaller<lambdaTest>(
    "scalarModelOneXCommonTest", &lambdaTest::scalarModelOneXCommonTest ) );
...
void lambdaTest::scalarModelNoXCommonTest()
{
  int nInd     = 1;
  int nXPerInd = 1;
  int nXCommon = 0;
...
void lambdaTest::scalarModelOneXCommonTest()
{
  int nInd     = 1;
  int nXPerInd = 0;
  int nXCommon = 1;
...
void lambdaTest::scalarModelTest( int nInd, int nXPerInd, int nXCommon )
{
  //------------------------------------------------------------
  ...
  FoMapsParSpkModel lambdaTestModel( &model, dvecAlpStep.toValarray() );
  ...
  double lambdaOut;
  DoubleMatrix drowLambda_xOut;
  DoubleMatrix drowLambda_alpOut;
  ...
  catch(...)
  {
    CPPUNIT_ASSERT_MESSAGE(
      "An unexpected exception occurred in lambda.",
      false );
  }

  try
  {
    drowLambda_xOut = lambda_alp( 
      lambdaTestModel,
      dvecY,
      dvecX,
      dvecAlp,
  ...
  {
    CPPUNIT_ASSERT_MESSAGE(
      "An unexpected exception occurred in lambda_alp.",
      false );
  }

  try
  {
    drowLambda_alpOut = lambda_b( 
      lambdaTestModel,
      dvecY,
      dvecX,
      dvecAlp,
  ...
  {
    CPPUNIT_ASSERT_MESSAGE(
      "An unexpected exception occurred in lambda_b.",
      false );
  }
  ...
  DoubleMatrix lambdaKnown;
  DoubleMatrix drowLambda_xKnown  ( 1, nX );
  DoubleMatrix drowLambda_alpKnown( 1, nAlp );
  ...
  //                     =  alp   -  log [ 1 - x  ]   .
  //
  lambdaKnown = pdAlp[0] * pdAlp[0] - log( 1.0 - pdX[0] * pdX[0] );
  drowLambda_xKnown  .fill( 2.0 * pdX[0] / ( 1.0 - pdX[0] * pdX[0] ) );
  drowLambda_alpKnown.fill( 2.0 * pdAlp[0] );
  ...
  CPPUNIT_ASSERT_MESSAGE(
    "lambdaOut not equal to lambdaKnown",
    relTolEqualTest(
      lambdaOut,
      lambdaKnown,
      tol,
  ...
  CPPUNIT_ASSERT_MESSAGE(
    "drowLambda_xOut not equal to drowLambda_xKnown",
    relTolEqualTest(
      drowLambda_xOut,
      drowLambda_xKnown,
      tol,
  ...
  CPPUNIT_ASSERT_MESSAGE(
    "drowLambda_alpOut not equal to drowLambda_alpKnown",
    relTolEqualTest(
      drowLambda_alpOut,
      drowLambda_alpKnown,
      tol,
