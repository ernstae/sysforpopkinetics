...
Test* hTildeTest::suite()
{
  TestSuite *suiteOfTests = new TestSuite( "hTildeTest" );

  suiteOfTests->addTest( new TestCaller<hTildeTest>(
    "scalarModelNoXCommonTest", &hTildeTest::scalarModelNoXCommonTest ) );

  suiteOfTests->addTest( new TestCaller<hTildeTest>(
    "scalarModelOneXCommonTest", &hTildeTest::scalarModelOneXCommonTest ) );
...
void hTildeTest::scalarModelNoXCommonTest()
{
  int nInd     = 1;
  int nXPerInd = 1;
  int nXCommon = 0;
...
void hTildeTest::scalarModelOneXCommonTest()
{
  int nInd     = 1;
  int nXPerInd = 0;
  int nXCommon = 1;
...
void hTildeTest::scalarModelTest( int nInd, int nXPerInd, int nXCommon )
{
  //------------------------------------------------------------
  ...
  DoubleMatrix dmatHTilde_alpOut( nAlp * nAlp, nAlp );
  ...
  catch(...)
  {
    CPPUNIT_ASSERT_MESSAGE(
      "An unexpected exception occurred in hTilde.",
      false );
  }
  ...
  DoubleMatrix dmatHTildeKnown    ( nAlp, nAlp );
  DoubleMatrix dmatHTilde_xKnown  ( nAlp * nAlp, nX );
  DoubleMatrix dmatHTilde_alpKnown( nAlp * nAlp, nAlp );
  ...
  //   HTilde(x, alp)  =  1 - x   .
  //
  dmatHTildeKnown    .fill( 1.0 - pdX[0] * pdX[0] );
  dmatHTilde_xKnown  .fill( -2.0 * pdX[0] );
  dmatHTilde_alpKnown.fill( 0.0 );
  ...
  CPPUNIT_ASSERT_MESSAGE(
    "dmatHTildeOut not equal to dmatHTildeKnown",
    relTolEqualTest(
      dmatHTildeOut,
      dmatHTildeKnown,
      tol,
  ...
  CPPUNIT_ASSERT_MESSAGE(
    "dmatHTilde_xOut not equal to dmatHTilde_xKnown",
    relTolEqualTest(
      dmatHTilde_xOut,
      dmatHTilde_xKnown,
      tol,
  ...
  CPPUNIT_ASSERT_MESSAGE(
    "dmatHTilde_alpOut not equal to dmatHTilde_alpKnown",
    relTolEqualTest(
      dmatHTilde_alpOut,
      dmatHTilde_alpKnown,
      tol,
