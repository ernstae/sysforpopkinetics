  ...
  : tree( treeIn ), canned_model( NONE ), isCannedModelUsed( false )
{
  ...
  DOMElement * modelTag = dynamic_cast<DOMElement*>(tree->getElementsByTagName( X("model" ) )->item(0));
...
const struct NonmemParameters NonmemSpkMLToCpp::getNonmemParameters() const
{
  return nonmem;
}
