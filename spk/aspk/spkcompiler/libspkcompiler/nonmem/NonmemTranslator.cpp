...
void NonmemTranslator::generateIndData( const SymbolTable & final_symbol_table ) const
{
  ...
  if( oIndData_h.good() )
      ...
      for( ; pLabel != labels->end(); pLabel++ )
	{
	  ...
	  //
	  oIndData_h << '\t' << "const std::valarray<" << (isID? "char*":"double") << ">";
      ...
      for( ; pRawTable != rawTable->end(); pRawTable++ )
	{
	    ...
	    {
	      oIndData_h << "std::valarray<double> " << pRawTable->second.name << ";" << endl;
	      oIndData_h << "std::valarray<double> " << pRawTable->second.name << "_tmp;" << endl;
	    }
	}
  ...
  //
  // Generate the definition file for IndData class.
  //
  ofstream oIndData_cpp( fIndData_cpp );
  if( oIndData_cpp.good() )
    {
      oIndData_cpp << BURNER << " <" << __FILE__ << ">" << endl;
      ...
      //
      oIndData_cpp << "IndData::IndData( int nIn";
      pLabel = labels->begin();
      for( ; pLabel != labels->end(); pLabel++ )
	{
          bool isID = ( *pLabel == pID->name );
	  ...
	  // Othewise, double.
	  //
	  oIndData_cpp << "const std::valarray<" << (isID? "char*":"double") << "> ";
          oIndData_cpp << "& " << *pLabel << "In";
	}
      oIndData_cpp << ")" << endl;
      oIndData_cpp << ": n( nIn )";
      ...
      for( ; pLabel != labels->end(); pLabel++ )
	{
	  ...
	  if( ( synonym = final_symbol_table.findi( *pLabel )->synonym ) != "" )
	    {
	      oIndData_cpp << "," << endl;
	      oIndData_cpp << synonym << "( " << *pLabel << "In" << " )";
	    }
	}
      ...
      for( ; pRawTable != rawTable->end(); pRawTable++ )
	{
	    if( find( labels->begin(), labels->end(), pRawTable->second.name ) == labels->end() )
	    {
	      ...
	      // computed values.
	      //
 	      oIndData_cpp << pRawTable->second.name << "( nIn )";
              oIndData_cpp << "," << endl;
	      ...
	      // computed values.
	      //
	      oIndData_cpp << pRawTable->second.name << "_tmp( nIn )";
	    }
	}
      ...
      oIndData_cpp << endl;
      oIndData_cpp << "IndData::~IndData(){}" << endl;
      oIndData_cpp << "IndData::IndData(){}" << endl;
      oIndData_cpp << "IndData::IndData( const IndData& ){}" << endl;
      oIndData_cpp << "IndData& IndData::operator=( const IndData& ){}" << endl;
    }
  else
    {
      assert( false );
...
{
  //
  // Generated the file burner for DataSet class.
  // 
  const Symbol* pID = final_symbol_table.findi("id");
  ...
  if( oDataSet_h.good() )
    {
      ...
      oDataSet_h << "std::vector<IndData*> dataset;" << endl;
      oDataSet_h << endl;
  ...
  //
  // Generated the .cpp file for DataSet class.
  // 
  ...
  if( oDataSet_cpp.good() )
    {
      oDataSet_cpp << BURNER << " <" << __FILE__ << ">" << endl;

      oDataSet_cpp << "#include <valarray>" << endl;
      ...
      oDataSet_cpp << ": dataset(" << pop_size << ")" << endl;
      oDataSet_cpp << "{" << endl;
      ...
      for( int who=0; who < pop_size; who++ )
	{
	  ...
	  for( int i=0; pLabel != labels->end(), i<nLabels; i++, pLabel++ )
	    {
	      const Symbol * s = final_symbol_table.findi( *pLabel );
              ...
              oDataSet_cpp << (isID? "char*":"double") << " " << carray_name << "[] = { ";
	      for( int j=0; j<nRecords; j++ )
		{
	      ...
	      oDataSet_cpp << " };" << endl;
	      oDataSet_cpp << "valarray<" << (isID? "char*":"double") << ">";
              oDataSet_cpp << valarray_name << "(" << carray_name << ", " << nRecords << ");" << endl;
	    }
	  ...
	  //
	  oDataSet_cpp << "dataset[" << who << "] = new IndData";
          oDataSet_cpp << "( " << nRecords << ", ";
	  pLabel = labels->begin();
	  for( int i=0; pLabel != labels->end(), i<nLabels; i++, pLabel++ )
	    {
	      if( i>0 )
