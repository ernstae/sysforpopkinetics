...
void read_nonmem_driverTest::setUp()
{
}
void read_nonmem_driverTest::tearDown()
{
}


void read_nonmem_driverTest::test()
{
  ...
  DOMElement * driverNode = dynamic_cast<DOMElement*>( 
	     parser->getDocument()
	     ->getElementsByTagName( XMLString::transcode( "driver" ) )->item(0) );
  const XMLCh * xml_analysis = 
    dynamic_cast<DOMElement*>( parser->getDocument()->getElementsByTagName( XMLString::transcode( "content" ) )->item(0) )
    ->getAttribute( XMLString::transcode( "analysis" ) );

  enum SpkParameters::Analysis analysis 
    = ( XMLString::equals( xml_analysis, XMLString::transcode( "population" ) )? 
			   SpkParameters::POPULATION : SpkParameters::INDIVIDUAL );
  CPPUNIT_ASSERT_MESSAGE( "The analysis type is supposed to be SpkParameters::POPULATION!",
			  analysis == SpkParameters::POPULATION );

  struct NonmemParameters nonmem;

  struct SpkParameters spk;

  read_nonmem_driver( driverNode, spk, nonmem );

  const int nIndividuals = 12;

  CPPUNIT_ASSERT_EQUAL_MESSAGE( "#of individuals is supposed to be 12!", 
			  nIndividuals, spk.nIndividuals );
...
CppUnit::Test * read_nonmem_driverTest::suite()
{
  CppUnit::TestSuite *suiteOfTests = new CppUnit::TestSuite( "read_nonmem_driverTest" );

  suiteOfTests->addTest( new CppUnit::TestCaller<read_nonmem_driverTest>("test",
						    &read_nonmem_driverTest::test ) );
