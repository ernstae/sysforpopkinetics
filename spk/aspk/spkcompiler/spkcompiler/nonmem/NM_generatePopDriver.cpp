...
void NonmemTranslator::generatePopDriver() const
{
  ...
  oPopDriver << "            valarray<double> fiTwoStage;" << endl;
  ...
  oPopDriver << "            valarray<double> thetaTwoStagePopMean( NonmemPars::nTheta );" << endl;
  oPopDriver << "            model.getTheta( thetaTwoStagePopMean );" << endl;
  oPopDriver << "            indModelForTwoStageMethod.setTheta( thetaTwoStagePopMean );" << endl;
  oPopDriver << endl;
  ...
  oPopDriver << "            int nTwoStageSigmaPar = model.getSigma().getNPar();" << endl;
  oPopDriver << "            valarray<double> omegaTwoStagePopMeanMinRep( nTwoStageSigmaPar );" << endl;
  oPopDriver << "            valarray<double> omegaTwoStagePopMean( NonmemPars::nEps * NonmemPars::nEps );" << endl;
  oPopDriver << "            model.getSigma( omegaTwoStagePopMeanMinRep );" << endl;
  oPopDriver << "            model.getSigma().expandCovMinRep( omegaTwoStagePopMeanMinRep, omegaTwoStagePopMean );" << endl;
  oPopDriver << "            indModelForTwoStageMethod.setOmega( omegaTwoStagePopMean );" << endl;
  oPopDriver << endl;
  oPopDriver << "            valarray<double> bTwoStagePopMean( nBTwoStage );" << endl;
  oPopDriver << "            indModelForTwoStageMethod.getIndPar( bTwoStagePopMean );" << endl;
  oPopDriver << endl;
  oPopDriver << "            valarray<double> twoStagePredOut;" << endl;
  oPopDriver << "            valarray<double> twoStageResOut; " << endl;
  oPopDriver << "            valarray<double> twoStageResWtdOut;" << endl;
  oPopDriver << "            valarray<double> twoStageResTruncatedOut; " << endl;
  oPopDriver << "            valarray<double> twoStageResWtdTruncatedOut;" << endl;
  oPopDriver << "            for( int i=0, j=0, k=0; i<nPop; j+=NObservs[i], k+=NRecords[i++] )" << endl;
  oPopDriver << "            {" << endl;
  oPopDriver << "               yiTwoStage.resize        ( NObservs[i] );" << endl;
  oPopDriver << "               twoStagePredOut.resize  ( NRecords[i] );" << endl;
  oPopDriver << "               twoStageResOut.resize   ( NRecords[i] );" << endl;
  oPopDriver << "               twoStageResWtdOut.resize( NRecords[i] );" << endl;
  oPopDriver << "               twoStageResTruncatedOut.resize   ( NObservs[i] );" << endl;
  oPopDriver << "               twoStageResWtdTruncatedOut.resize( NObservs[i] );" << endl;
  oPopDriver << "               yiTwoStage = y[ slice( j, NObservs[i], 1 ) ]; " << endl;
  ...
  oPopDriver << "                                bTwoStagePopMean," << endl;
  oPopDriver << "                                pVANull," << endl;
  oPopDriver << "                               &twoStageResTruncatedOut," << endl;
  oPopDriver << "                               &twoStageResWtdTruncatedOut, " << endl;
  oPopDriver << "                                pVANull, " << endl;
  oPopDriver << "                                pVANull );" << endl;
  oPopDriver << endl;
  oPopDriver << "                  dataForDisposal.expand( i, twoStageResTruncatedOut,    twoStageResOut );" << endl;
  oPopDriver << "                  dataForDisposal.expand( i, twoStageResWtdTruncatedOut, twoStageResWtdOut );" << endl;
  oPopDriver << "               }" << endl;
  oPopDriver << "               catch( SpkException& e )" << endl;
  oPopDriver << "               {" << endl;
  oPopDriver << "                  char message[SpkError::maxMessageLen()];" << endl;
  ...
  oPopDriver << "                  ret = STATISTICS_FAILURE;" << endl;
  oPopDriver << "               }" << endl;
  oPopDriver << "               resOut      [ slice( k, NRecords[i], 1 ) ]  = twoStageResOut;" << endl;
  oPopDriver << "               resWtdOut   [ slice( k, NRecords[i], 1 ) ]  = twoStageResWtdOut;" << endl;
  oPopDriver << "            }" << endl;
  oPopDriver << "            // Set the PRED values here so that they have values for" << endl;
  oPopDriver << "            // all of the data records.  Note that the OdePred" << endl;
  oPopDriver << "            // evaluator only saves values after the last individuals'" << endl;
  ...
  oPopDriver << "               for( int j=0; j<NRecords[i]; j++)" << endl;
  oPopDriver << "               {" << endl;
  oPopDriver << "                  scalarToDouble( dataForDisposal.data[i]->PRED[j], twoStagePredOut[j] );" << endl;
  oPopDriver << "               }" << endl;
  oPopDriver << "               predOut[ slice( k, NRecords[i], 1 ) ]  = twoStagePredOut;" << endl;
  oPopDriver << "            }" << endl;
  ...
  oPopDriver << "            valarray<double> iTwoStageResTruncatedOut; " << endl;
  oPopDriver << "            valarray<double> iTwoStageResWtdTruncatedOut;" << endl;
  oPopDriver << "            for( int i=0, j=0, k=0; i<nPop; j+=NObservs[i], k+=NRecords[i++] )" << endl;
  oPopDriver << "            {" << endl;
  oPopDriver << "               yiTwoStage.resize        ( NObservs[i] );" << endl;
  oPopDriver << "               fiTwoStage.resize        ( NObservs[i] );" << endl;
  oPopDriver << "               iTwoStagePredOut.resize  ( NRecords[i] );" << endl;
  oPopDriver << "               iTwoStageResOut.resize   ( NRecords[i] );" << endl;
  oPopDriver << "               iTwoStageResWtdOut.resize( NRecords[i] );" << endl;
  oPopDriver << "               iTwoStageResTruncatedOut.resize   ( NObservs[i] );" << endl;
  oPopDriver << "               iTwoStageResWtdTruncatedOut.resize( NObservs[i] );" << endl;
  oPopDriver << "               yiTwoStage = y[ slice( j, NObservs[i], 1 ) ]; " << endl;
  ...
  oPopDriver << "                  // Make sure each individual's data mean values has" << endl;
  oPopDriver << "                  // been calculated at the optimal parameter values." << endl;
  oPopDriver << "                  indModelForTwoStageMethod.dataMean( fiTwoStage );" << endl;
  oPopDriver << endl;
  oPopDriver << "                  // Don't calculate the individual's parameter residuals" << endl;
  oPopDriver << "                  // or the individual's PRED values." << endl;
  oPopDriver << "                  valarray<double>* pVANull = 0;" << endl;
  ...
  oPopDriver << "                                pVANull," << endl;
  oPopDriver << "                               &iTwoStageResTruncatedOut," << endl;
  oPopDriver << "                               &iTwoStageResWtdTruncatedOut, " << endl;
  oPopDriver << "                                pVANull, " << endl;
  oPopDriver << "                                pVANull );" << endl;
  oPopDriver << endl;
  oPopDriver << "                  dataForDisposal.expand( i, iTwoStageResTruncatedOut,    iTwoStageResOut );" << endl;
  oPopDriver << "                  dataForDisposal.expand( i, iTwoStageResWtdTruncatedOut, iTwoStageResWtdOut );" << endl;
  oPopDriver << "               }" << endl;
  oPopDriver << "               catch( SpkException& e )" << endl;
  oPopDriver << "               {" << endl;
  oPopDriver << "                  char message[SpkError::maxMessageLen()];" << endl;
  ...
  oPopDriver << "                  ret = STATISTICS_FAILURE;" << endl;
  oPopDriver << "               }" << endl;
  oPopDriver << "               iResOut      [ slice( k, NRecords[i], 1 ) ]  = iTwoStageResOut;" << endl;
  oPopDriver << "               iResWtdOut   [ slice( k, NRecords[i], 1 ) ]  = iTwoStageResWtdOut;" << endl;
  oPopDriver << "            }" << endl;
  oPopDriver << "            // Set the IPRED values here so that they have values for" << endl;
  oPopDriver << "            // all of the data records.  Note that the OdePred" << endl;
  oPopDriver << "            // evaluator only saves values after the last individuals'" << endl;
  ...
  oPopDriver << "               for( int j=0; j<NRecords[i]; j++)" << endl;
  oPopDriver << "               {" << endl;
  oPopDriver << "                  scalarToDouble( dataForDisposal.data[i]->PRED[j], iTwoStagePredOut[j] );" << endl;
  oPopDriver << "               }" << endl;
  oPopDriver << "               iPredOut[ slice( k, NRecords[i], 1 ) ]  = iTwoStagePredOut;" << endl;
  oPopDriver << "            }" << endl;
  oPopDriver << "            // The two-stage methods do not provide values for the eta" << endl;
  oPopDriver << "            // values that appear in the population model, so just set" << endl;
  oPopDriver << "            // these residuals all equal to zero." << endl;
