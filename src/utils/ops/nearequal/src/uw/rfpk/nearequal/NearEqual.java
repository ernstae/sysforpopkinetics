...
//                                    aTol = s * (rErr * (Math.abs(DV) + Math.abs(F)) + aErr);
        ...
        double oldTime = 0;
        double newTime = 0;
        if(oldOutput.computingTimes != null)
        {
            if(newOutput.computingTimes != null)
            {
                for(int i = 0; i < oldOutput.computingTimes.length; i++)
                {
                    if(oldOutput.computingTimes[i] != null && !oldOutput.computingTimes[i].equals(""))
                        oldTime += Double.parseDouble(oldOutput.computingTimes[i]);
                }
                for(int i = 0; i < newOutput.computingTimes.length; i++)
                {
                    if(newOutput.computingTimes[i] != null && !newOutput.computingTimes[i].equals(""))
                        newTime += Double.parseDouble(newOutput.computingTimes[i]);
                }
            }
            else
                msg.append("\nElapsed time is missing.");
        }
        
        if(msg.length() == 0)
        {
            System.out.print("No difference. ");
            System.out.print("Old time: " + oldTime + " New time: " + newTime);
        }
        else
        {
            ...
            if(paramOnly)
            {
                System.out.println(getParamReport(oldOutput, newOutput));
            }
            System.exit(1);
        }
    }
    ...
    {  
        ...
        String omega = "";
        if(oldOutput.omega != null && oldOutput.omegaStruct != null)
        {
            for(int l = 0; l < oldOutput.omega.length; l++)
            {
                omega = "OMEGA\n";
                if(oldOutput.omegaStruct[l].equals("block"))
                {
                    for(int j = 1; j < oldOutput.omega[l].length + 1; j++)
                    {
               ...
               if(oldOutput.omegaStruct[l].equals("diagonal"))
               {
        ...
        String sigma = "";
        if(oldOutput.sigma != null && oldOutput.sigmaStruct != null)
        {
            for(int l = 0; l < oldOutput.sigma.length; l++)
            {
                sigma = "SIGMA\n";
                if(oldOutput.sigmaStruct[l].equals("block"))
                {
                    for(int j = 1; j < oldOutput.sigma[l].length + 1; j++)
                    {
                ...
                if(oldOutput.sigmaStruct[l].equals("diagonal"))
                {
